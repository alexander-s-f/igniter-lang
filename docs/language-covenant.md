...
| P16 | Non-idempotent operations under retry are a compile error | `accepted (scoped)` | PROP-048 OOF-PROF4 (`LANG-PROFILE-IDEMPOTENCY-RETRY-P31`) — a retry-enabled profile bound to a contract declaring `idempotency none` is a live hard compile error (classifier), Ruby-canon like the PROF family; broader idempotency-key requiredness on every retry path remains pending (PROP-035). Row corrected from stale `planned PROP` by `LANG-CAPABILITY-DECLARATION-ARTIFACT-PARITY-IMPLEMENTATION-P3` (R3 census finding); no enforcement change made by that correction |
| P17 | Irreversible contracts name their compensation or declare `no_compensation` | `partial` | ch12 required-fields slice — `compensation` declared-or-waived is the live dual `OOF-M3` warning (never blocks acceptance); `compensation_mode`/`compensation_ref` parse and emit dual since the P22 slice; hard requiredness (OOF-M17-class) remains HELD. Row corrected from stale `planned PROP` by `LANG-CAPABILITY-DECLARATION-ARTIFACT-PARITY-IMPLEMENTATION-P3` (R3 census finding); no enforcement change made by that correction |
