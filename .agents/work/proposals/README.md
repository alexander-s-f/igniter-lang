...
| [LANG-STDLIB-SUM](LANG-STDLIB-SUM-field-projection-sum-v0.md) | authored-pending-review | `stdlib.collection.sum` two-arg field-projection form only: `sum(Collection[T], Symbol) -> F` where F = declared type of Symbol-named field in T; one-arg form BLOCKED (absent from stdlib spec; Rust scale-stripping bug; requires LAB-STDLIB-NUMERIC-P1); return type is structural field lookup — scale-preserving for Decimal[N] without arithmetic type inference; bare source alias `sum` accepted; non-Symbol second arg → OOF-COL5; missing field → OOF-COL5; OOF-COL1 arity / OOF-COL2 non-Collection first arg; empty-collection runtime value deferred to P2; sumBy explicitly deferred; fold-derivation noted (independent implementation); Ruby TC: OOF-TY0 for both forms (P2 adds infer_sum_call + @type_shapes lookup); Rust TC: two-arg scale-preserving (working) + SIR bare "sum" name (parity gap); semantic_ir_name==canonical_name; no legacy_sir; grounded by LAB-STDLIB-SUM-P1 46/46 PASS (SPLIT-NUMERIC Split A) + stdlib/collections.ig spec + stdlib_extension.ig conformance fixture; next LANG-STDLIB-SUM-PROP-P2 implementation planning |
