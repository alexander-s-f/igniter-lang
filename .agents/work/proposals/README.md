...
| [PROP-IMPORT-RESOLUTION](PROP-IMPORT-RESOLUTION-import-resolution-and-multifile-compilation-unit-v0.md) | authored — proposal only | `import` = compile-time name resolution only; N `.ig` files form one logical compilation universe and one `.igapp`; OOF-M1 circular import, OOF-M2 unknown import, OOF-M3 duplicate module; deterministic multi-file `source_hash`; import grants no capability/profile/package/runtime authority; package/stdlib/visibility/implementation deferred |
