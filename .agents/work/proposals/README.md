...
| [PROP-IMPORT-RESOLUTION](PROP-IMPORT-RESOLUTION-import-resolution-and-multifile-compilation-unit-v0.md) / [P2](PROP-IMPORT-RESOLUTION-P2-implementation-planning-v0.md) / [P2A](PROP-IMPORT-RESOLUTION-P2A-diagnostic-namespace-decision-v0.md) | authored + implementation planning + diagnostic namespace accepted | `import` = compile-time name resolution only; N `.ig` files form one logical compilation universe and one `.igapp`; P2 selects compiler-driver pre-pass + merged logical universe; P2A reserves `OOF-IMP*` for import/module diagnostics (`OOF-IMP1..5`) and leaves existing modifier/effect/profile/evidence `OOF-M*` meanings untouched; import grants no capability/profile/package/runtime authority; package/stdlib/visibility/implementation deferred; next P3 may use final diagnostic mapping if separately authorized |
